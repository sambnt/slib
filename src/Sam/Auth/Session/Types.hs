...
data SessionResult a = SessionExpired
                     | SessionNotFound
                     | SessionFound a
...
data SessionConfig =
  SessionConfig { sessionTimeoutIdle :: TimeoutSecondsIdle
...
data NewSession a = NewSession { newSessionId :: SessionId
                               , newSessionCreatedAt :: Time
                               , newSessionData :: a
                               }
  deriving (Eq, Show)

data GetSession = GetSession { getSessionId :: SessionId
                             , getSessionAt :: Time
                             }
  deriving (Eq, Show)

data EndSession = EndSession { endSessionId :: SessionId
                             , endSessionAt :: Time
                             }
  deriving (Eq, Show)

data Session a b = SessionAnonymous (SessionData a)
...
data SessionStore m anon auth
  = SessionStore { newSession          :: SessionConfig -> NewSession anon -> m (Maybe (SessionData anon))
                 , authenticateSession :: SessionConfig -> SessionId -> NewSession auth -> m (Maybe (SessionResult (SessionData auth)))
                 , getSession          :: SessionConfig -> GetSession -> m (SessionResult (Session anon auth))
                 , endSession          :: SessionConfig -> EndSession -> m ()
                 }

data Anonymous = Anonymous { anonSessionRedirect :: Maybe URI
                           , anonSessionCodeVerifier :: Text
                           }
  deriving (Eq, Show)

data Authenticated = Authenticated { authUserClaims :: UserClaims
                                   }
  deriving (Eq, Show)
