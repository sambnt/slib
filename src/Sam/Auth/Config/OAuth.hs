...
import Network.OAuth2.Experiment (AuthorizationCodeApplication (AuthorizationCodeApplication), ClientId (ClientId), ClientSecret (ClientSecret), ClientAuthenticationMethod (ClientSecretPost), Idp (..), IdpApplication (IdpApplication))
import Sam.Util.URI (fromURIByteString, URI)
...
data ConfigOAuth =
  ConfigOAuth { cfgOAuthApp :: forall i. IdpApplication i AuthorizationCodeApplication
  ...
  let
    ...
    app =
      AuthorizationCodeApplication
        ...
        Map.empty
        ClientSecretPost
  pure $ ConfigOAuth { cfgOAuthApp = IdpApplication idProvider app
