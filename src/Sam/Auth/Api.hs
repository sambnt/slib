...
import Sam.Auth.OAuth (OAuth, pkceTokenRequest, startOAuth, logoutURI)
import Sam.Auth.Session.Cookies
    ( Cookies(..),
      ...
      authenticateSession,
      endSession )
import Sam.Auth.Session.Types (SessionResult(..), Session(..), sessionData, sessionId, anonSessionRedirect)
import Sam.Util.URI (URI, parseURI, uriToStr)
import Servant (Get, Proxy (Proxy), Server, Handler, Header, (:>), errHeaders, throwError, errBody, err500, err302, err404)
import Servant.API (AuthProtect, QueryParam', Required, Strict, (:-), NamedRoutes)
import Servant.Server (err400)
import Servant.Server.Experimental.Auth (AuthServerData, AuthHandler, mkAuthHandler)
import Web.Cookie (renderSetCookieBS, parseCookies)
...
apiServer oauth sessions jwks pool =
  AuthAPI' { callback = callbackHandler oauth sessions jwks pool
...
logoutHandler oauth sessions pool cs = do
  ...
  throwError $ err302 {
    errHeaders = [ ("Location", maybe "/" uriToStr $ logoutURI oauth)
  ...
  case mSesh of
    ...
    SessionFound (SessionAnonymous sd) -> do
      ...
      case eToken of
        Left err ->
          throwError $ err500 {
            errBody = "Unable to exchange token with OAuth2 server: " <> BSLU.fromString (show err)
          }
        Right token -> do
          case idToken token of
            Nothing -> do
              throwError $ err500 {
                errBody = "OAuth2 server did not return 'idToken' in: " <> BSLU.fromString (show token)
              }
            Just idT -> do
              ...
              case eUserClaims of
                ...
                Right userClaims -> do
                  sr <- liftIO $ flip runSqlPool pool $
                  ...
                  case sr of
                    SessionFound (_auth, setCookie) -> do
                      ...
                      throwError $ err302 {
                        errHeaders = [ ("Location", redirectUri)
                                     , ("Set-Cookie", renderSetCookieBS setCookie)
                                     ]
                        }
                    _ -> do
...
beginSession oauth sessions pool mcs redirectTo = do
  ...
  throwError $ err302 {
    errHeaders = [ ("Location", uriToStr signInURI)
                 , ("Set-Cookie", renderSetCookieBS setCookie)
                 ]
    }
